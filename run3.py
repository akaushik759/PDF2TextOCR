...
def isValidRow(row):
	...
	if hasDecimalValueOrStar(row):
		if not startsWithANumber(row):
			return endsWithUnits(row) or endsWithANumber(row)
	return endsWithUnits(row)

#Current Directory path
cur_dir_path = os.path.dirname(os.path.realpath("Least_Important.pdf"))


# Path of the pdf 
dir_path = os.path.join(os.path.dirname(os.path.realpath("Least_Important.pdf")),"Least_Important.pdf")
