...
def isValidRow(row):
	...
	if hasDecimalValue(row):
		if not startsWithANumber(row):
			...
			return (endsWithUnits(row) or endsWithANumber(row)) and not hasGender(row)
	return False

#Current Directory path
cur_dir_path = os.path.dirname(os.path.realpath("Important.pdf"))


# Path of the pdf 
dir_path = os.path.join(os.path.dirname(os.path.realpath("Important.pdf")),"Important.pdf")
